...
const Specialization = () => {
  ...
  const t = useThemeTokens(); // ← hook ở đây
          ...
          <div style={s.grid}>
            {specialties.map((item) => (
              <div
                ...
                style={{
                  ...s.card,
                  ...(selected?.key === item.key ? s.cardActive : {}),
                }}
              ...
              >
                <div style={{
                  ...s.iconBox,
                  background: selected?.key === item.key ? "#0a6abf" : t.brandBg,
                }}>
                ...
                <div style={s.cardText}>
                  <div style={{
                    ...s.cardName,
                    color: selected?.key === item.key ? "#0a6abf" : t.textPrimary,
                  }}>
                    {item.name}
                  </div>
                  <div style={s.cardDesc}>{item.desc}</div>
                </div>
                <div style={{ ...s.arrow, opacity: selected?.key === item.key ? 1 : 0 }}>→</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <>
          ...
          <div style={s.panel}>
            ...
            <div style={s.panelInfo}>
              {[
                ["📋", "Về chuyên khoa", selected.desc],
                ["⏰", "Giờ làm việc", "Thứ 2 – Thứ 7  |  7:30 – 17:00"],
                ["💰", "Chi phí khám", "200,000 – 500,000 VNĐ"],
              ].map(([icon, label, val]) => (
...
const getStyles = (t) => ({
  ...
  panelIcon:   { width: 52, height: 52, background: "rgba(255,255,255,0.15)", borderRadius: 13, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, flexShrink: 0 },
